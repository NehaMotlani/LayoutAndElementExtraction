...
if __name__ == '__main__':
    ...
    char2idx_path = root + 'data/char2idx.json'
    target_path = root + 'data/target.json'
    target2idx_path = root + 'data/target2idx.json'
    mask_generator.load_config(corpus_path, char2idx_path, target_path, target2idx_path)

    # --------- LOAD MODEL ----------------
    model_path = root + 'data/weights/model_epoch_17.pth'
    ...
    for img_path in glob.glob(root + 'data/test/images/*.png'):
        image_name = os.path.basename(img_path).replace('.png', '')

        # image_name = 'X51006557194'
        image_path = os.path.join(root, 'data/test/images', image_name + '.png')
        label_path = os.path.join(root, 'data/test/labels', image_name + '.json')
        ...
        # --------- MAIN ---------
        print(image_name)
        data = information_extraction(image, label_data)
        with open(root + 'data/output/'+image_name+'.json', 'w') as outfile:
