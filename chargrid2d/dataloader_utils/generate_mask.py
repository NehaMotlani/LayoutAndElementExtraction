...
# import utils
import sys
root = '/home/pushpa/Documents/research/funsd_chargrid/funsd_chargrid/chargrid2d-icdar/chargrid2d/'
sys.path.insert(1, '/home/pushpa/Documents/research/funsd_chargrid/funsd_chargrid/chargrid2d-icdar/chargrid2d/')
...
class MaskGenerator():
    ...
    def __make_corpus(self):
        ...
        for idx, item in enumerate(sorted_x):
            ...
            if len(self.corpus) > 300:
                break
        write_json(root+'data/corpus.json', self.corpus)
        write_json(root+'data/char2idx.json', self.char2idx)
    ...
    def convert_lbl(self, lbl_fol, std_lbl_fol):
        ...
        for idx, file_path in enumerate(all_files):
            ...
            #print("name and img folder",name,img_fol)
            lbl_path = file_path
    ...
    def __generate_object(self, img_path, lbl_path):
        ...
        for item in lbl_data:
            ...
            #print(item['value'],"w","h","length",w,h,len(item['value']))
            ...
            for char in item['value']:
                ...
                #print("self.target2idx[cl]",self.target2idx[cl])
                cur_x += char_w
            ...
            #print(int(item['location'][0]), int(item['location'][1]), int(item['location'][2]), int(item['location'][3]))
            obj.append(std_item)
    ...
    def generate_mask(self, out_fol):
        for lbl_path, img_path in zip(self.path_std_lbls, self.path_imgs):
            ...
            name = name.split('.')[0]
            #print("obj_gt",obj_gt)
            cv2.imwrite(osp.join(root+'data/debug/', name + '.png'), debug_img)
            cv2.imwrite(osp.join(root+'data/semantic_gt/', name + '.png'), semantic_gt)
            write_json(osp.join(root+'data/obj_gt/', name + '.json'), obj_gt)
            torch.save(chargrid, osp.join(root+'data/tensor_input/', name + '.pt'))
    ...
    def generate_target(self):
        ...
        write_json(root+'data/target.json', self.target)
        write_json(root+'data/target2idx.json', self.target2idx)
    ...
    def process(self, lbl_fol, img_fol, out_fol):
        make_folder(out_fol)
        make_folder(root+'data/debug')
        make_folder(root+'data/semantic_gt')
        make_folder(root+'data/tensor_input')
        make_folder(root+'data/obj_gt')
    ...
    def generate_test_file(self, image, label_dict):
        ...
        for item in label_dict:
            ...
            char_w, char_h = int(w / (len(item['text']) + 1)), int(h)
...
#     root = 'D:/cinnamon/dataset/kyocera/S3/data/20190924'
    ...
    root = '/home/pushpa/Documents/research/funsd_chargrid/funsd_chargrid/chargrid2d-icdar/'

    lbl_fol = osp.join(root+'data', 'label_temp')
    img_fol = osp.join(root+'data', 'img_temp')
    # print(lbl_fol,img_fol)
    out_fol = osp.join(root+'data', 'standard_lbl')
