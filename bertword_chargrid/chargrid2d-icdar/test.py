...
if __name__ == '__main__':
    ...
    char2idx_path = root+'/data/word2idx.json'
    target_path = root+'/data/target.json'
    target2idx_path = root+'/data/target2idx.json'
    print(corpus_path,char2idx_path)
    mask_generator.load_config(corpus_path, char2idx_path, target_path, target2idx_path)

    # --------- LOAD MODEL ----------------
    model_path = root+'/data/weights/model_epoch_24.pth'
    model = Chargrid2D(768, len(mask_generator.target))
