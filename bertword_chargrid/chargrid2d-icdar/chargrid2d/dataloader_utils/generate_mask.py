...
class MaskGenerator():
    ...
    def __convert_data_sroie(self, label_json):
        ...
        for item in label_json:
            if item['text'] not in self.__corpus__:
                self.__corpus__[item['text']] = 1
            else:
                self.__corpus__[item['text']] += 1
    ...
    def __make_corpus(self):
        ...
        for idx, item in enumerate(sorted_x):
            ...
            self.word2idx[item[0]] = idx
            # if len(self.corpus) > 300:
            #     break
        write_json(root+'/data/corpus.json', self.corpus)
        write_json(root+'/data/word2idx.json', self.word2idx)

        return self.corpus
    
    def get_word2idx(self, word):
        if word in self.word2idx:
            return self.word2idx[word] + 1
        else:
            return 0
    ...
    def __generate_object(self, img_path, lbl_path):
        ...
        for item in lbl_data:
            ...
            mask[cur_y: cur_y + h, cur_x: cur_x + w] = self.get_word2idx(item['value'])
            gt[cur_y: cur_y + h, cur_x: cur_x + w] = self.target2idx[cl]
                # cur_x += char_w
    ...
    def generate_mask(self, out_fol):
        for lbl_path, img_path in zip(self.path_std_lbls, self.path_imgs):
            ...
            print('here')
            print(chargrid.size(0),chargrid.size(1))
            name = osp.basename(img_path)
            name = name.split('.')[0]
            print(name)
    ...
    def load_config(self, corpus_path, word2idx_path, target_path, target2idx_path):
        self.corpus = self.load_json(corpus_path)
        self.word2idx = self.load_json(word2idx_path)
    ...
    def generate_test_file(self, image, label_dict):
        ...
        for item in label_dict:
            ...
            mask[cur_y: cur_y + h, cur_x: cur_x + w] = self.get_word2idx(item['text'])
                # cur_x += char_w
...
#     root = 'D:/cinnamon/dataset/kyocera/S3/data/20190924'
    ...
    lbl_fol = osp.join(root, 'data/words')
    ...
    mask_generator.generate_mask(out_fol)
